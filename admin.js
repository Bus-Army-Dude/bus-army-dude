...
function convertTimeToTimezoneBH_Admin(timeStr, targetTimezone) {
    ...
    } else { console.warn("Could not parse time for conversion:", timeStr); return "Invalid Time"; } // Return specific error for bad format
...
function checkOpenStatus_Admin(dayOfWeek, todayDateStr, regularHoursData, holidayFormData, tempClosureFormData) {
    ...
    if (holidayFormData && holidayFormData.date === todayDateStr && holidayFormData.name && holidayFormData.hours) {
         ...
         if (hoursTodayStr.toLowerCase() === 'closed') return { status: "Closed", reason: reason, hoursToday: "Closed", isHoliday: true }; // Added isHoliday flag
         const parts = hoursTodayStr.split(" - ");
         if (parts.length === 2) { effectiveOpenStr = parts[0].trim(); effectiveCloseStr = parts[1].trim(); }
         else { return { status: "Closed", reason: `${reason} (Invalid hours format)`, hoursToday: hoursTodayStr, isHoliday: true }; }
    } else {
        ...
        if (!effectiveOpenStr || effectiveOpenStr.toLowerCase() === 'closed' || !effectiveCloseStr || effectiveCloseStr.toLowerCase() === 'closed') {
            return { status: "Closed", reason: reason, hoursToday: "Closed", isHoliday: false };
        }
    }

    // Helper to parse time string (HH:MM AM/PM or HH:MM 24hr) into total minutes
    const parseTimeToMinutesEST = (timeStr) => { if (!timeStr || typeof timeStr !== 'string') return -1; timeStr = timeStr.trim().toUpperCase(); if (timeStr === 'CLOSED') return -1; const timeRegex12hr = /^(0?[1-9]|1[0-2]):([0-5][0-9])\s?(AM|PM)$/i; const timeRegex24hr = /^(\d{1,2}):([0-5][0-9])$/; let hours = -1, minutes = -1; if (timeRegex12hr.test(timeStr)) { const match = timeStr.match(timeRegex12hr); hours = parseInt(match[1], 10); minutes = parseInt(match[2], 10); const period = match[3].toUpperCase(); if (period === 'PM' && hours !== 12) hours += 12; if (period === 'AM' && hours === 12) hours = 0; } else if (timeRegex24hr.test(timeStr)) { const match = timeStr.match(timeRegex24hr); hours = parseInt(match[1], 10); minutes = parseInt(match[2], 10); } else { console.warn("Invalid time format for parsing:", timeStr); return -1; } if (hours < 0 || hours > 23 || minutes < 0 || minutes > 59) return -1; return hours * 60 + minutes; };

    const openMinutes = parseTimeToMinutesEST(effectiveOpenStr);
    const closeMinutes = parseTimeToMinutesEST(effectiveCloseStr);
    if (openMinutes === -1 || closeMinutes === -1) { return { status: "Closed", reason: reason, hoursToday: hoursTodayStr, isHoliday: isHoliday }; }
    ...
    if (tempClosureFormData && tempClosureFormData.date === todayDateStr && tempClosureFormData.startTime && tempClosureFormData.endTime && tempClosureFormData.reason) {
        const fromMinutes = parseTimeToMinutesEST(tempClosureFormData.startTime);
        const toMinutes = parseTimeToMinutesEST(tempClosureFormData.endTime);
        if(fromMinutes !== -1 && toMinutes !== -1 && fromMinutes < toMinutes) { // Added check for valid times
            let isCurrentlyClosed;
            if (toMinutes < fromMinutes) { isCurrentlyClosed = (currentTotalMinutesEST >= fromMinutes) || (currentTotalMinutesEST < toMinutes); }
            else { isCurrentlyClosed = currentTotalMinutesEST >= fromMinutes && currentTotalMinutesEST < toMinutes; }
            if (isCurrentlyClosed) {
                isTempClosed = true;
                activeTempClosure = tempClosureFormData;
            }
        }
    }
    if (isTempClosed && activeTempClosure) { return { status: "Temporarily Unavailable", reason: activeTempClosure.reason, hoursToday: hoursTodayStr, tempClosureDetails: activeTempClosure, isHoliday: isHoliday }; }
...
function updateBusinessInfoPreview() {
    ...
    const currentHolidayData = { date: holidayDateInput?.value || null, name: holidayNameInput?.value.trim() || null, hours: holidayHoursInput?.value.trim() || null };
    ...
    const currentTempClosureData = { date: tempDateInput?.value || null, startTime: tempStartInput?.value || null, endTime: tempEndInput?.value || null, reason: tempReasonInput?.value.trim() || null };
    ...
    const todayDateStr = currentDate.toLocaleDateString("en-CA", { timeZone: 'America/New_York' }); // Use EST/EDT date for comparison

    // Check status using form data
    const openStatusResult = checkOpenStatus_Admin( currentDay, todayDateStr, currentRegularHours, currentHolidayData, currentTempClosureData );
    ...
    if (hoursContainer) {
        ...
        daysOrder.forEach(day => {
            ...
            const openTimeEST = currentRegularHours?.[openKey] || "Closed"; const closeTimeEST = currentRegularHours?.[closeKey] || "Closed";
            ...
            const dayElement = document.createElement("div"); dayElement.className = "preview-hours-row"; if (day === currentDay) { dayElement.classList.add("current-day"); }
...
businessHoursInputs.forEach(input => {
    if (input) {
        // Update on 'input' for text fields, 'change' for date/time
    ...
    if (statusElement) { statusElement.textContent = openStatusResult.status; statusElement.className = 'preview-open-status'; statusElement.classList.add(openStatusResult.status.toLowerCase().replace(/\s+/g, "-")); }
    ...
    if (holidayAlert && openStatusResult.isHoliday && currentHolidayData.date === todayDateStr) {
        if (holidayName) holidayName.textContent = currentHolidayData.name; if (holidayHours) holidayHours.textContent = currentHolidayData.hours; holidayAlert.style.display = "block";
    } else if (holidayAlert) { holidayAlert.style.display = "none"; }
    ...
    if (tempAlert && openStatusResult.status === "Temporarily Unavailable" && openStatusResult.tempClosureDetails) {
         const tempClosure = openStatusResult.tempClosureDetails; if(tempReason) tempReason.textContent = tempClosure.reason;
         const convertedStart = convertTimeToTimezoneBH_Admin(tempClosure.startTime, userTimezone); const convertedEnd = convertTimeToTimezoneBH_Admin(tempClosure.endTime, userTimezone);
         if(tempHours) tempHours.textContent = `${convertedStart} - ${convertedEnd}`; tempAlert.style.display = "block";
    } else if (tempAlert) { tempAlert.style.display = "none"; }
}
