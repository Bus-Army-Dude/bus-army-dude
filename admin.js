// admin.js (Includes Inactivity Logout + Timer Display, Corrected Paths, Counts, Next Button Logic, and Direct Links)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadShoutoutsAdmin(platform) {
        ...
        try {
            ...
            // NOTE: Firestore requires an index for compound queries (where + orderBy different field)
            // If you haven't created one, the console will show an error with a link to create it.
             const shoutoutQuery = query(shoutoutsCol, where("platform", "==", platform), orderBy("order", "asc")); // Filter by platform and order
            // const querySnapshot = await getDocs(collection(db, 'shoutouts')); // Old way - inefficient loading

            const querySnapshot = await getDocs(shoutoutQuery); // Use the filtered query
