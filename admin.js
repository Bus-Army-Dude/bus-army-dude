...
// ======================================================
// ===== START: CORRECTED Admin Preview Functions v3 ====
// ======================================================
...
// --- Admin Preview Function (Using Minutes Comparison + Added Logging) ---
function updateAdminPreviewBI() {
    console.log("%cUpdating admin preview...", "color: blue; font-weight: bold;"); // Log start
    ...
    if (!businessInfoForm || !adminPreviewStatus || !adminPreviewHours || !adminPreviewContact || !statusOverrideSelect || !contactEmailInput) {
        console.error("Admin preview update failed: One or more required HTML elements missing!");
    ...
    const daysOfWeek = ['monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday']; // Define locally if not global
    ...
    // console.log("Admin Preview - Form Data:", currentFormData); // Optional: Log form data
    ...
    const previewDateStr = previewNow.toLocaleDateString('en-CA'); // YYYY-MM-DD
    const previewCurrentMinutes = previewNow.getHours() * 60 + previewNow.getMinutes();
    console.log(`Admin Preview - Current Time: Date=${previewDateStr}, Day=${previewDayName}, Mins=${previewCurrentMinutes}`);
    ...
    if (!ruleApplied) {
        ...
        if (activeTemporary) {
            ...
            ruleApplied = true; // Temporary rule applies for today, override regular hours
            console.log("Admin Preview Status determined by: Temporary Rule");

            if (activeTemporary.isClosed) { // Explicitly closed for the whole period
                currentStatus = 'Temporarily Unavailable';
                console.log(">>> Admin Preview Temp Check: Rule is marked 'Closed' -> Temporarily Unavailable");
                ...
                if (openMins === null || closeMins === null) { // Invalid times provided
                    ...
                    console.log(`>>> Admin Preview Temp Check Condition Result: (Current >= Open && Current < Close) = ${isWithin}`); // Log boolean result
                    if (isWithin) {
                        currentStatus = 'Open'; // WITHIN specific temp hours
                         console.log(">>> Admin Preview Temp Check Final Status: Open");
                    } else {
                        currentStatus = 'Temporarily Unavailable'; // OUTSIDE specific temp hours
                        console.log(">>> Admin Preview Temp Check Final Status: Temporarily Unavailable");
                    }
                }
            } else { // Temporary rule exists but no specific times AND not marked closed? Treat as unavailable.
                 currentStatus = 'Temporarily Unavailable';
