...
import { getFirestore, collection, addDoc, getDocs, doc, deleteDoc, updateDoc, setDoc, serverTimestamp, getDoc, query, orderBy, where, limit, runTransaction } from "https://www.gstatic.com/firebasejs/10.10.0/firebase-firestore.js";
...
async function handleAddAlert(event, alertType) {
    ...
    Okay, you've found the exact error message! loadAlertsAdmin is not defined confirms that the code inside your handleAddAlert function is trying to call loadAlertsAdmin() to refresh the alert lists, but that function doesn't actually exist in your admin.js file.

You also have a related issue: the delete buttons created in renderAlertList call a function handleDeleteAlert, which also needs to be defined.

Here's how to fix this:

1. Define the loadAlertsAdmin Function:

This function needs to fetch the businessInfo document (which contains both activeHolidays and activeTemporary arrays) and then render both lists.

Add this new function definition somewhere alongside your other loading functions (like loadProfileData, loadShoutoutsAdmin, etc.) inside the main DOMContentLoaded listener:

JavaScript

    // ========================================================
    // NEW: Function to Load and Display Alerts in Admin Panel
    // ========================================================
    async function loadAlertsAdmin() {
        // Use the container element consts defined earlier
        if (!holidayAlertsListContainer || !temporaryAlertsListContainer) {
            console.error("Alert list container elements not found.");
            showAdminStatus("UI Error: Cannot display alerts.", true);
            return;
        }
        if (!businessInfoDocRef) {
             console.error("Business info doc ref missing for loading alerts.");
             showAdminStatus("Config Error: Cannot load alerts.", true);
             return;
        }

        // Show loading state
        holidayAlertsListContainer.innerHTML = '<p>Loading holiday alerts...</p>';
        temporaryAlertsListContainer.innerHTML = '<p>Loading temporary alerts...</p>';
        ...
        try {
            ...
            if (docSnap.exists()) {
                const data = docSnap.data();
                console.log("Loaded alert data from businessInfo:", data);
                // Ensure arrays exist, default to empty array if field is missing/null
                holidayAlerts = Array.isArray(data.activeHolidays) ? data.activeHolidays : [];
                temporaryAlerts = Array.isArray(data.activeTemporary) ? data.activeTemporary : [];
            } else {
                console.warn(`Business info document ('${businessInfoDocRef.path}') not found while loading alerts.`);
                 // Still render empty lists
            }

            // Render both lists using the existing render function
            renderAlertList(holidayAlerts, holidayAlertsListContainer, 'holiday');
            renderAlertList(temporaryAlerts, temporaryAlertsListContainer, 'temporary');

            console.log(`Displayed ${holidayAlerts.length} holiday and ${temporaryAlerts.length} temporary alerts.`);

        } catch (error) {
            console.error("Error loading alerts for admin panel:", error);
            holidayAlertsListContainer.innerHTML = '<p class="error">Error loading holiday alerts.</p>';
            temporaryAlertsListContainer.innerHTML = '<p class="error">Error loading temporary alerts.</p>';
            showAdminStatus("Error loading active alerts.", true);
        }
    }
    // ========================================================
    // END Load Alerts Function
    // ========================================================

    // ========================================================
    // NEW: Function to Handle Deleting an Alert
    // ========================================================
    async function handleDeleteAlert(alertId, alertType) {
         if (!alertId || !alertType) {
             console.error("Missing alert ID or type for deletion.");
             showAdminStatus("Error: Cannot delete alert (missing info).", true);
             return;
         }

         const confirmationMessage = `Are you sure you want to permanently delete this ${alertType} alert? This cannot be undone.`;
         ...
         try {
             // Use a transaction to safely read and update the array
             await runTransaction(db, async (transaction) => {
                 const docSnap = await transaction.get(businessInfoDocRef);
                 if (!docSnap.exists()) {
                     throw new Error("Business info document not found.");
                 }

                 const data = docSnap.data();
                 const fieldName = (alertType === 'holiday') ? 'activeHolidays' : 'activeTemporary';
                 const currentAlerts = Array.isArray(data[fieldName]) ? data[fieldName] : [];
                 ...
                 if (updatedAlerts.length === currentAlerts.length) {
                      console.warn(`Alert with ID ${alertId} not found in ${fieldName} array during deletion.`);
                      // Optionally throw an error or just proceed
                      // throw new Error(`Alert with ID ${alertId} not found.`);
                 }

                 // Update the document with the modified array
                 transaction.set(businessInfoDocRef, { [fieldName]: updatedAlerts }, { merge: true });
             });

             console.log(`${alertType} alert with ID ${alertId} deleted successfully.`);
             showAdminStatus(`${alertType.charAt(0).toUpperCase() + alertType.slice(1)} alert deleted successfully!`, false);

             // Refresh the lists in the admin panel AFTER successful deletion
             loadAlertsAdmin(); // <<<--- Call the load function here

         } catch (error) {
             console.error(`Error deleting ${alertType} alert (ID: ${alertId}):`, error);
             showAdminStatus(`Error deleting alert: ${error.message}`, true);
         }
     }
    // ========================================================
    // END Delete Alert Function
    // ========================================================


    // *** ADD THIS BLOCK TO LOAD ALERTS ***
            if (typeof loadAlertsAdmin === 'function') { // Check if function exists
                 loadAlertsAdmin(); // Load initial holiday and temporary alerts
            } else {
                 console.error("loadAlertsAdmin function missing!");
                 showAdminStatus("Error: Cannot load business alerts section.", true);
            }
            // *** END BLOCK TO ADD ***
...
// --- ADD THIS FUNCTION ---
   ...
   // Function to Load Disabilities into the Admin Panel List (CORRECTED)
    async function loadDisabilitiesAdmin() {
        ...
        disabilitiesListAdmin.innerHTML = `<p>Loading disability links...</p>`; // Show loading message initially
        ...
        try {
            ...
            // STEP 1: Populate the global array ONLY
            querySnapshot.forEach((doc) => {
                allDisabilities.push({ id: doc.id, ...doc.data() }); // Store data
                // STEP 2: Ensure direct render call is REMOVED from here
            });
            ...
            if (typeof displayFilteredDisabilities === 'function') {
                displayFilteredDisabilities(); // This will clear loading message and render
            } else {
                console.error("displayFilteredDisabilities function not defined!");
                if (disabilitiesListAdmin) disabilitiesListAdmin.innerHTML = `<p class="error">Error initializing display.</p>`;
                if (disabilitiesCount) disabilitiesCount.textContent = '(Error)';
            }

        } catch (error) {
            console.error("Error loading disability links:", error);
            allDisabilities = []; // Clear array on error

            // Display specific error message in the container
            if (disabilitiesListAdmin) {
                if (error.code === 'failed-precondition') {
                    disabilitiesListAdmin.innerHTML = `<p class="error">Error: Missing Firestore index. See console (F12).</p>`;
        ...
        // STEP 4: Ensure NO calls to displayFilteredDisabilities() are here
    }
