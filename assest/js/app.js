...
settingsBtn?.addEventListener('click', () => {
    tempSelect.value = Settings.current.units.temperature;
    speedSelect.value = Settings.current.units.speed;
    pressureSelect.value = Settings.current.units.pressure;
    themeToggle.checked = Settings.current.display.theme === 'dark';
    timeToggle.checked = Settings.current.display.time24;
    locationToggle.checked = Settings.current.privacy.location;
...
settingsSave?.addEventListener('click', () => {
    Settings.current.units.temperature = tempSelect.value;
    Settings.current.units.speed = speedSelect.value;
    Settings.current.units.pressure = pressureSelect.value;
    Settings.current.display.theme = themeToggle.checked ? 'dark' : 'light';
    Settings.current.display.time24 = timeToggle.checked;
    Settings.current.privacy.location = locationToggle.checked;
...
function updateWeatherDisplay() {
    document.querySelectorAll('[data-temp]').forEach(el => {
        const rawTemp = el.dataset.temp;
        if (rawTemp) {
            const converted = Settings.convert(
                parseFloat(rawTemp),
                'temperature',
                'celsius',
                Settings.current.units.temperature
            );
            el.textContent = Settings.format(converted, 'temperature', Settings.current.units.temperature);
        }
    });

    document.querySelectorAll('[data-wind]').forEach(el => {
        const rawSpeed = el.dataset.wind;
        if (rawSpeed) {
            const converted = Settings.convert(
                parseFloat(rawSpeed),
                'speed',
                'ms',
                Settings.current.units.speed
            );
            el.textContent = Settings.format(converted, 'speed', Settings.current.units.speed);
        }
    });

    document.querySelectorAll('[data-pressure]').forEach(el => {
        const rawPressure = el.dataset.pressure;
        if (rawPressure) {
            const converted = Settings.convert(
                parseFloat(rawPressure),
                'pressure',
                'hpa',
                Settings.current.units.pressure
            );
            el.textContent = Settings.format(converted, 'pressure', Settings.current.units.pressure);
        }
    });
}
