...
        pressure: "hpa", // Corrected to match the option value
        timeFormat24: false // Add default for 12-hour format
    };
...
    }
    // Add for time format
    const timeFormatToggle = document.querySelector("[data-settings-time-format]");
    if (timeFormatToggle) {
        timeFormatToggle.checked = settings.timeFormat24;
    }
};
...
    const pressureUnitControl = document.querySelector("[data-settings-pressure]");
    const themeToggle = document.querySelector("[data-settings-theme]");
    const timeFormatToggle = document.querySelector("[data-settings-time-format]"); // Add this line
...
    }
    ...
    // Add event listener for time format toggle
    if (timeFormatToggle) {
        timeFormatToggle.addEventListener("change", () => {
            const is24Hour = timeFormatToggle.checked;
            const currentSettings = JSON.parse(localStorage.getItem("weatherSettings")) || {};
            currentSettings.timeFormat24 = is24Hour;
            localStorage.setItem("weatherSettings", JSON.stringify(currentSettings));
            applySettings(currentSettings); // Re-apply settings to update the time display
        });
    }
});
