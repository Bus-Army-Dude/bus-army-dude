...
let techItemsCollectionRef; // Reference for Tech Items
...
try {
    ...
    // Display a general error on the body if Firebase fails
...
function formatFirestoreTimestamp(firestoreTimestamp) {
    ...
    try {
        ...
        // More robust locale string options
        return date.toLocaleString(navigator.language || 'en-US', { // Use browser language or fallback
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    disabilitiesListPlaceholder = document.getElementById('disabilities-list-placeholder'); // Ensure this ID exists in HTML
    presidentPlaceholderElement = document.getElementById('president-placeholder'); // Ensure this ID exists in HTML
    techItemsListContainer = document.getElementById('tech-items-list-dynamic'); // Ensure this ID exists in HTML
    ...
    tiktokTimestampEl = document.getElementById('tiktok-last-updated-timestamp'); // Ensure this ID exists
    instagramTimestampEl = document.getElementById('instagram-last-updated-timestamp'); // Ensure this ID exists
    youtubeTimestampEl = document.getElementById('youtube-last-updated-timestamp'); // Ensure this ID exists

    if (!firebaseAppInitialized) {
        console.error("Firebase not ready. Site cannot load.");
        // Show error message if Firebase didn't initialize
        if (maintenanceMessageElement) {
            ...
            // Fallback if maintenance message div isn't found
            const errorDiv = document.createElement('div');
            errorDiv.className = 'error-message-fallback'; // Add a class for potential styling
            errorDiv.innerHTML = '<p class="error" style="text-align: center; color: red; padding: 20px;">Site cannot load (Connection Error).</p>';
    ...
    try {
        ...
        if (configSnap.exists()) {
            ...
            console.warn("Site config document ('site_config/mainProfile') not found. Assuming maintenance mode is OFF.");
        }
        ...
        if (maintenanceEnabled) {
            ...
            if (maintenanceMessageElement) {
                maintenanceMessageElement.innerHTML = '<p>The site is currently undergoing maintenance. Please check back later.</p>'; // User-friendly message
                maintenanceMessageElement.style.display = 'block';
             } else {
                 console.error("Maintenance message element (#maintenanceModeMessage) is missing from HTML!");
                 // Add a fallback message directly to body if needed
                 const maintDiv = document.createElement('div');
                 maintDiv.innerHTML = '<p style="text-align:center; padding: 50px; color: orange;">Site Maintenance</p>';
                 document.body.prepend(maintDiv);
             }
             // Don't load content if in maintenance mode
             return;
        } else {
            ...
            // Use Promise.allSettled to load sections concurrently and handle individual errors
            const loadPromises = [
            ...
            // Optional: Log results of loading each section
             results.forEach((result, index) => {
                ...
                if (result.status === 'rejected') {
                    ...
                    // console.log(`${functionNames[index]} loaded successfully.`); // Can uncomment for verbose success logs
                }
            });
            console.log("All dynamic content loading initiated.");
        }
    } catch (error) {
        // General error handling during DOMContentLoaded (e.g., maintenance check failure)
        console.error("Error during DOMContentLoaded initialization or maintenance check:", error);
        if (maintenanceMessageElement) {
            maintenanceMessageElement.innerHTML = `<p class="error">An error occurred while loading site configuration: ${error.message}. Please try again later.</p>`;
            maintenanceMessageElement.style.display = 'block';
        } else {
             const errorDiv = document.createElement('div');
             errorDiv.innerHTML = `<p class="error" style="text-align: center; color: red; padding: 20px;">An error occurred loading site configuration.</p>`;
             document.body.prepend(errorDiv);
        }
        if (mainContentWrapper) mainContentWrapper.style.display = 'none'; // Hide content on config error
    }
