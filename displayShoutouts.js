// displayShoutouts.js (Corrected with Useful Links + Social Links Logic)
...
async function loadAndDisplayShoutouts() { //
    if (!firebaseAppInitialized || !db) { //
        ...
        if(tGrid) tGrid.innerHTML = '<p class="error" style="color: red;">Error: DB connection failed.</p>'; // Corrected selector
        if(iGrid) iGrid.innerHTML = '<p class="error" style="color: red;">Error: DB connection failed.</p>'; // Corrected selector
        if(yGrid) yGrid.innerHTML = '<p class="error" style="color: red;">Error: DB connection failed.</p>'; // Corrected selector
        return; //
    }
        ...
        if (querySnapshot.empty) {
            ...
            querySnapshot.forEach((doc) => {
                ...
                if (data.label && data.url) {
                    const linkElement = document.createElement('a');
                    linkElement.href = data.url;
                    linkElement.textContent = data.label; // Display the label as text
                    linkElement.target = '_blank'; // Open in new tab
                    linkElement.rel = 'noopener noreferrer'; // Security best practice
                    linkElement.className = 'social-link-button'; // Add a class for styling (you can define this class in your CSS)

                    // Optional: Add an icon if you have a mapping or store icon classes in Firestore
                    // const iconElement = document.createElement('i');
                    // iconElement.className = 'fab fa-some-icon'; // Replace with actual icon class
                    // linkElement.prepend(iconElement); // Add icon before the label
